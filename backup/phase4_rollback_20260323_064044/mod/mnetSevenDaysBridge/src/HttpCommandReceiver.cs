...
namespace mnetSevenDaysBridge
{
    public sealed class HttpCommandReceiver : IDisposable
    {
        ...
        public HttpCommandReceiver(
            ...
            ObservationCommandQueue observationCommandQueue)
        {
        ...
        private bool TryHandleGet(Stream stream, string path, Dictionary<string, string> query)
        {
            switch (path.ToLowerInvariant())
            {
                ...
                case "api/get_capabilities":
                    ...
                    return true;
                case "api/get_state":
                    WriteResponse(stream, 200, BuildSuccess("get_state", ExecuteObservationCommand("get_state", null)));
                    return true;
                case "api/get_player_position":
                    WriteResponse(stream, 200, BuildSuccess("get_player_position", ExecuteObservationCommand("get_player_position", null)));
                    return true;
                case "api/get_player_rotation":
                    WriteResponse(stream, 200, BuildSuccess("get_player_rotation", ExecuteObservationCommand("get_player_rotation", null)));
                    return true;
                case "api/get_logs_tail":
                    ...
                    return true;
                case "api/get_look_target":
                    WriteResponse(stream, 200, BuildSuccess("get_look_target", ExecuteObservationCommand("get_look_target", null)));
                    return true;
                case "api/get_interaction_context":
                    WriteResponse(stream, 200, BuildSuccess("get_interaction_context", ExecuteObservationCommand("get_interaction_context", null)));
                    return true;
                case "api/query_resource_candidates":
                    WriteResponse(stream, 200, BuildSuccess("query_resource_candidates", ExecuteObservationCommand("query_resource_candidates", ToObjectArguments(query))));
                    return true;
                case "api/query_interactables_in_radius":
                    WriteResponse(stream, 200, BuildSuccess("query_interactables_in_radius", ExecuteObservationCommand("query_interactables_in_radius", ToObjectArguments(query))));
                    return true;
                case "api/query_entities_in_radius":
                    WriteResponse(stream, 200, BuildSuccess("query_entities_in_radius", ExecuteObservationCommand("query_entities_in_radius", ToObjectArguments(query))));
                    return true;
                case "api/get_environment_summary":
                    WriteResponse(stream, 200, BuildSuccess("get_environment_summary", ExecuteObservationCommand("get_environment_summary", null)));
                    return true;
                case "api/get_biome_info":
                    WriteResponse(stream, 200, BuildSuccess("get_biome_info", ExecuteObservationCommand("get_biome_info", null)));
                    return true;
                case "api/get_terrain_summary":
                    WriteResponse(stream, 200, BuildSuccess("get_terrain_summary", ExecuteObservationCommand("get_terrain_summary", null)));
                    return true;
                default:
                    return false;
            }
        ...
        private object ExecuteCommand(string command, Dictionary<string, object> arguments)
        {
            switch ((command ?? string.Empty).Trim().ToLowerInvariant())
            {
                ...
                case "get_state":
                    return ExecuteObservationCommand("get_state", null);
                case "get_player_position":
                    return ExecuteObservationCommand("get_player_position", null);
                case "get_player_rotation":
                    return ExecuteObservationCommand("get_player_rotation", null);
                case "get_logs_tail":
                    var requestedLines = 50;
                ...
                case "get_look_target":
                    return ExecuteObservationCommand("get_look_target", arguments);
                case "get_interaction_context":
                    return ExecuteObservationCommand("get_interaction_context", arguments);
                case "query_resource_candidates":
                    return ExecuteObservationCommand("query_resource_candidates", arguments);
                case "query_interactables_in_radius":
                    return ExecuteObservationCommand("query_interactables_in_radius", arguments);
                case "query_entities_in_radius":
                    return ExecuteObservationCommand("query_entities_in_radius", arguments);
                case "get_environment_summary":
                    return ExecuteObservationCommand("get_environment_summary", arguments);
                case "get_biome_info":
                    return ExecuteObservationCommand("get_biome_info", arguments);
                case "get_terrain_summary":
                    return ExecuteObservationCommand("get_terrain_summary", arguments);
                default:
        ...
        private object ExecuteObservationCommand(string commandName, Dictionary<string, object> arguments)
        {
            observationCommandQueue.TryEnqueue(commandName, arguments, out var queuedCommand);
            if (!queuedCommand.Completion.Wait(config.RequestTimeoutMs))
            {
                throw new BridgeCommandException(504, "command_timeout", "Timed out waiting for the observation command to execute on the main thread.");
            }
        ...
        private static Dictionary<string, object> ToObjectArguments(Dictionary<string, string> query)
        {
            ...
            foreach (var pair in query)
            {
                ...
                if (bool.TryParse(pair.Value, out var boolValue))
                {
                    values[pair.Key] = boolValue;
                    continue;
                }

                if (int.TryParse(pair.Value, NumberStyles.Integer, CultureInfo.InvariantCulture, out var intValue))
                {
                    values[pair.Key] = intValue;
                    continue;
                }

                if (float.TryParse(pair.Value, NumberStyles.Float, CultureInfo.InvariantCulture, out var floatValue))
                {
                    values[pair.Key] = floatValue;
                    continue;
                }
